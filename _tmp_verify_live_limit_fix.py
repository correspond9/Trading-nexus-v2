...
try:
    ...
    print(run_cmd(ssh, f"docker exec {backend} sh -lc \"grep -n 'quantity         = \(limit_remaining_qty if ord_type == \"LIMIT\" else qty\)' /app/app/routers/orders.py\""))
    print(run_cmd(ssh, f"docker exec {backend} sh -lc \"grep -n 'marketable now at top-book' /app/app/execution_simulator/partial_fill_monitor.py\""))

...
Token is auto-refreshed by TokenRefresher using TOTP ΓÇö no manual rotation
...
# ΓöÇΓöÇ In-memory cache (avoids DB round-trip on every WS connect) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
_client_id:    str = ""
_access_token: str = ""
...
# ΓöÇΓöÇ Phase 10: Encryption key (from environment or generate) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
async def load_credentials() -> None:
    ...
    log.info(
        f"Credentials loaded ΓÇö client_id={'set' if _client_id else 'EMPTY'}, "
...
async def rotate_token(
    ...
    *,
    ...
    # Persist expiry (upsert ΓÇö row may or may not exist yet)
    if expiry is not None:
        await pool.execute(
            """
...
def is_token_expiring_soon(within_minutes: int = 120) -> bool:
    ...
    if _token_expiry is None:
        return True  # Unknown expiry ΓåÆ assume refresh needed

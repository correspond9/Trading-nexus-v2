...
=============================================
Core execution engine ΓÇö routes orders to fill/queue/reject logic.
Persists results to paper_orders, paper_positions, paper_trades, execution_log.
Called by:
  - API router (new order)
  - tick_processor (on every market tick ΓÇö checks queued limit orders)
"""
import asyncio
import logging
import uuid
...
# Shared mock_mode flag ΓÇö set by Admin router at runtime
...
# ΓöÇΓöÇ Main entry point: place_order ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
async def place_order(order, user_id: str, market_snap: dict) -> dict:
    ...
    """
    if not _mock_mode:
        raise RuntimeError("Execution engine is not in PAPER mode ΓÇö live trading not implemented.")

    order_id = str(uuid.uuid4())
    tick_size = get_tick_size(order.exchange_segment)

    # ΓöÇΓöÇ Fetch lot size from instrument master ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    pool     = get_pool()
    ...
    # ΓöÇΓöÇ Pre-flight rejection check (synchronous) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    # ΓöÇΓöÇ MARKET order: fill immediately from current depth ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    # ΓöÇΓöÇ LIMIT / SL order: queue for deferred fill ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    queued = QueuedOrder(
        order_id         = order_id,
        user_id          = user_id,
...
# ΓöÇΓöÇ Called by tick_processor on every tick flush ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
async def on_tick(instrument_token: int, market_snap: dict) -> None:
    ...
    Called up to 10 times/sec ΓÇö must be fast (optimistic lock check).
    """
    if not _mock_mode:
        return
...
# ΓöÇΓöÇ Cancel an open order ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ

...
===================================
ATM strike calculation ΓÇö deterministic and rule-driven.
...
# Cache: underlying ΓåÆ current ATM strike
...
def calculate_atm(underlying_ltp: float, strike_step: float) -> Decimal:
    ...
    Example:
        ltp=22347.5, step=50 ΓåÆ ATM=22350
    """
...
def update_atm(underlying: str, ltp: float, strike_step: float) -> Decimal:
    ...
    if prev != atm:
        log.info(
            f"ATM updated ΓÇö {underlying}: {prev} ΓåÆ {atm}  "
...
def generate_strike_range(
    ...
    num_strikes_each_side = 50 ΓåÆ 101 total (ATM ┬▒ 50)
    num_strikes_each_side = 25 ΓåÆ 51 total
    """
    step   = Decimal(str(strike_step))

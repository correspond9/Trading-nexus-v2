...
Tier-A (on-demand ΓÇö stock options):
  - NSE Stock Options (208 stocks ├ù 2 expiries ├ù 51 strikes ├ù 2 = ~42k tokens)

WS slot assignment (Tier-B):
  ws_slot = hash(instrument_token) % 5  ΓåÆ  deterministic, even distribution
"""
import csv
import hashlib
import logging
...
# ΓöÇΓöÇ Paths ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
def _ws_slot(instrument_token: int) -> int:
    """Deterministic slot 0-4 via modulo ΓÇö same token always maps to same WS."""
...
async def load_instruments() -> None:
    ...
    Called once at startup (idempotent ΓÇö uses ON CONFLICT DO UPDATE).
    """
    ...
    log.info(f"Upserting {len(batch)} instruments into instrument_masterΓÇª")
    ...
    log.info(f"Instruments loaded ΓÇö Tier-B: {tier_b}, Tier-A: {tier_a}")
...
def _classify_row(
    ...
    try:
        ...
        # ΓöÇΓöÇ Classification ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ

        # NSE Index Options ΓÇö Tier-B

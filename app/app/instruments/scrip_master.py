...
Admin API
---------
`get_list_csv(list_name)` ΓåÆ CSV bytes (for download endpoint)
`replace_list_from_csv(list_name, content)` ΓåÆ replaces rows and reclassifies
"""
import asyncio
import csv
import io
...
# Local CSV files for initial seeding  (list_name ΓåÆ path, name_column, is_display_name)
_LOCAL_LIST_FILES = {
...
def _ws_slot(token: int) -> int:
    """Deterministic WS slot 0-4 ΓÇö same token always maps to same slot."""
    return token % 5


# ΓöÇΓöÇ Classification ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
def _classify(row: dict, lists: dict[str, set[str]]) -> Optional[str]:
    ...
    # ΓöÇΓöÇ Index options & futures ΓÇö always Tier-B ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    # ΓöÇΓöÇ Stock options ΓÇö Tier-A (on-demand) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    if itype == "OPTSTK" and underlying in lists[LIST_OPTIONS_STOCKS] and exch_id == "NSE" and seg_code == "D":
        return "A"

    # ΓöÇΓöÇ Stock futures ΓÇö Tier-B ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    if itype == "FUTSTK" and underlying in lists[LIST_FUTURES_STOCKS] and exch_id == "NSE" and seg_code == "D":
        return "B"

    # ΓöÇΓöÇ Equity cash ΓÇö Tier-B ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    if itype == "EQUITY" and underlying in lists[LIST_EQUITY] and exch_id == "NSE" and seg_code == "E":
        return "B"

    # ΓöÇΓöÇ ETF ΓÇö Tier-B ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    # ΓöÇΓöÇ MCX commodity futures ΓÇö Tier-B ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    if itype == "FUTCOM" and underlying in lists[LIST_MCX_FUTURES] and exch_id == "MCX" and seg_code == "M":
        return "B"

    # ΓöÇΓöÇ MCX commodity options ΓÇö Tier-B ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
# ΓöÇΓöÇ Row parsing ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
# ΓöÇΓöÇ Subscription lists ΓÇö DB I/O ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ

async def load_subscription_lists_from_db() -> dict[str, set[str]]:
    """Returns mapping list_name ΓåÆ set of uppercase UNDERLYING_SYMBOL."""
    pool = get_pool()
...
async def seed_subscription_lists_if_empty() -> None:
    ...
    if total > 0 and all((counts.get(name, 0) > 0) for name in ALL_LISTS):
        log.info(f"Subscription lists already seeded ({total} rows) ΓÇö skipping.")
        return

    if not LOCAL_CSV.exists():
        log.warning(
            "Local scrip master CSV not found ΓÇö cannot seed subscription lists. "
            f"Expected: {LOCAL_CSV}"
        )
        return

    log.info("Seeding subscription lists from local CSV filesΓÇª")

    # Build display_name.upper() ΓåÆ UNDERLYING_SYMBOL from local master CSV
    ...
    for list_name, (filename, col, is_display_name) in _LOCAL_LIST_FILES.items():
        ...
        with open(path, newline="", encoding="utf-8", errors="replace") as f:
            ...
            for row in reader:
                ...
                if is_display_name:
                    # Resolve display name ΓåÆ UNDERLYING_SYMBOL
    ...
    if rows_to_insert:
        ...
        log.info(f"Subscription lists seeded ΓÇö {len(rows_to_insert)} rows inserted.")


# ΓöÇΓöÇ Instrument upsert ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
# ΓöÇΓöÇ Core refresh logic ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
async def _refresh_from_content(content: str) -> None:
    ...
    log.info(
        f"instrument_master refreshed ΓÇö "
...
async def refresh_instruments(download: bool = True) -> None:
    """
    Main entry point.
    download=True  ΓåÆ fetch fresh CSV from DhanHQ CDN (production)
    download=False ΓåÆ read local CSV file (dev mode / first run fallback)
    """
    if download:
        log.info(f"Downloading scrip master from {CDN_URL} ΓÇª")
        try:
            ...
            log.error(f"CDN download failed: {exc} ΓÇö falling back to local CSV.")
            download = False
...
# ΓöÇΓöÇ Admin helpers ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
async def replace_list_from_csv(list_name: str, content: str) -> int:
    ...
    log.info(f"Replaced {list_name} list with {len(symbols)} symbols. Re-classifyingΓÇª")
...
async def _reclassify_in_place() -> None:
    ...
    This is a fast in-DB operation only ΓÇö used after an admin list upload.
...
class ScripMasterScheduler:
    ...
    async def start(self) -> None:
        ...
        log.info("ScripMasterScheduler started ΓÇö will refresh at 06:00 IST daily.")
    ...
    async def _loop(self) -> None:
        while True:
            try:
                ...
                log.info("ScripMasterScheduler: triggering daily scrip master refreshΓÇª")

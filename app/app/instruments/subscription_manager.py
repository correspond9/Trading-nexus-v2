...
# ws_slot ΓåÆ set of instrument_tokens currently subscribed
_active: dict[int, set[int]] = {i: set() for i in range(5)}

# instrument_token ΓåÆ ws_slot  (for fast reverse lookup)
_token_to_slot: dict[int, int] = {}

_lock = asyncio.Lock()


# ΓöÇΓöÇ Tier-B startup ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ

async def initialise_tier_b() -> None:
    """
    Load all Tier-B tokens from DB into the subscription map.
    Called once at startup ΓÇö ws_manager.subscribe_all() reads _active.
    """
    pool = get_pool()
    ...
    log.info(
        f"Tier-B subscription map ready ΓÇö {total} tokens across 5 WS slots: "
        + ", ".join(f"WS-{i}: {len(_active[i])}" for i in range(5))
    )


# ΓöÇΓöÇ Tier-A on-demand ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
async def subscribe_tier_a(instrument_token: int) -> int | None:
    ...
    Idempotent ΓÇö calling twice for the same token is safe.
    """
    async with _lock:
...
async def unsubscribe_tier_a(instrument_token: int) -> None:
    """
    Unsubscribe a Tier-A token ΓÇö only if it is expired AND
    has no watchlist entry and no open position.
    """
    if not await _is_safe_to_unsubscribe(instrument_token):
        log.debug(
            f"Token {instrument_token} kept ΓÇö still in watchlist or has open position."
        )
        return
...
# ΓöÇΓöÇ Expiry rollover ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ

async def handle_expiry_rollover() -> None:
    """
    Called daily after market close.
    Evaluates all active Tier-A tokens ΓÇö unsubscribes those that are
...
def get_stats() -> dict:
    """Summary of current subscription state ΓÇö used by Admin Dashboard."""
    return {
        "slots": [
        ...
        "tier_a_tokens": sum(
            ...
            if slot is not None  # rough proxy ΓÇö can refine with tier lookup
        ),
    }

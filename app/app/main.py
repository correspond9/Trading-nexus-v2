...
Lifespan: ordered startup sequence ΓåÆ graceful shutdown.
"""
import logging
...
async def lifespan(app: FastAPI):
    ...
    Startup sequence (order matters):
      ...
      4.  Instrument master seed/refresh
            ΓåÆ DHAN DISABLED: reads local CSV only
            ΓåÆ DHAN ENABLED:  downloads fresh CSV from DhanHQ CDN
     ...
     12.  NSE margin data initial load (SPAN + ELM) ΓÇö always enabled
     12b. MCX and BSE margin data initial load
     13.  NSE margin daily scheduler (08:45 IST) ΓÇö always enabled
     14.  Market hours updated with cached holidays
     15.  Super Order monitor (Target + SL + Trailing)
    """
    try:
        log.info("ΓöÇΓöÇΓöÇ Startup ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ")

        log.info("[1] Initialising database pool + running migrationsΓÇª")
        await init_db()

        log.info("[1b] Syncing trading holidays from exchange websitesΓÇª")
        try:
            holidays_synced = await sync_exchange_holidays()
            if holidays_synced:
                log.info("[1b] Γ£ô Trading holidays synced from NSE, MCX, BSE")
            else:
        ...
        log.info("[2] Loading DhanHQ credentials from DBΓÇª")
        await load_credentials()

        if is_static_configured():
            log.info("[2a] Static IP credentials detected ΓÇö setting auth_mode=static_ip")
            ...
            try:
                ...
                if resp.status_code == 200:
                    ...
                    log.warning(
                        "[2a] Static IP auth verification failed (HTTP %s) ΓÇö "
                ...
                log.warning(
                    "[2a] Static IP auth verification error (%s) ΓÇö falling back to auto_totp.",
                    exc,
                )
                await set_auth_mode("auto_totp")
        else:
            log.info("[2a] Static IP credentials not present ΓÇö setting auth_mode=auto_totp")
            await set_auth_mode("auto_totp")

        log.info("[2b] Starting static IP auth monitorΓÇª")
        await static_auth_monitor.start()

        log.info("[3] Seeding subscription lists from local CSV filesΓÇª")
        await seed_subscription_lists_if_empty()

        if cfg.dhan_disabled:
            log.info("[4] Loading instrument master from local CSV (Dhan disabled)ΓÇª")
            await refresh_instruments(download=False)

            log.info("[10] Building option chain skeleton (Dhan disabled)ΓÇª")
            await greeks_poller.build_skeleton()

            log.warning(
                "[ΓÜá] DHAN CONNECTIONS DISABLED ΓÇö token refresher, WebSocket managers, "
                "Greeks poller, and daily CDN refresh are all off. "
                "Set DISABLE_DHAN_WS=false + STARTUP_START_STREAMS=true to enable."
            )
        else:
            log.info("[4] Downloading fresh instrument master from DhanHQ CDNΓÇª")
            ...
            if not cfg.startup_start_streams:
                log.warning(
                    "[ΓÜá] STARTUP_START_STREAMS=false ΓÇö skipping Dhan stream startup. "
                    "Use SuperAdmin ΓåÆ Connect to Dhan to start streams at runtime."
                )
            elif not (get_client_id() and get_access_token()):
                log.warning(
                    "[ΓÜá] No Dhan credentials in DB ΓÇö skipping Dhan stream startup. "
                    "Save Client ID + Access Token in SuperAdmin, then Connect to Dhan."
                )
            else:
                log.info("[5] Starting TOTP token refresherΓÇª")
                await token_refresher.start()

                if cfg.startup_load_tier_b:
                    log.info("[6] Initialising Tier-B subscriptionsΓÇª")
                    await subscription_manager.initialise_tier_b()
                else:
                    log.warning(
                        "[6] STARTUP_LOAD_TIER_B=false ΓÇö skipping Tier-B init at startup. "
                        "(Connect to Dhan will initialise on demand.)"
                    )

                log.info("[7] Starting tick processorΓÇª")
                await tick_processor.start()

                log.info("[8] Starting Live Feed WebSocket manager (5 slots)ΓÇª")
                await ws_manager.start_all()

                log.info("[9] Starting Full Depth WebSocket managerΓÇª")
                pool = get_pool()
                ...
                log.info("[10] Building option chain skeleton + starting Greeks pollerΓÇª")
                await greeks_poller.build_skeleton()
                await greeks_poller.start()

                log.info("[11] Starting daily scrip master scheduler (06:00 IST)ΓÇª")
                await scrip_scheduler.start()

        log.info("[12] Loading NSE margin data (SPAN┬« + Exposure Limit) ΓÇª")
        try:
            # First try loading from database cache
            db_loaded = await load_margin_from_db()
            if db_loaded:
                log.info("[12] Γ£ô Loaded NSE SPAN margin data from database cache")
        ...
        log.info("[12b] Loading MCX and BSE margin data ΓÇª")
        try:
            # Load MCX margins from database cache
            await mcx_margin_data._load_latest_from_db()
            log.info("[12b] Γ£ô Loaded MCX margin data from database cache")
        except Exception as exc:
        ...
        try:
            ...
            log.info("[12b] Γ£ô Loaded BSE margin data from database cache")
        except Exception as exc:
            log.warning(f"[12b] BSE margin load failed ({exc})")

        log.info("[13] Starting NSE margin daily scheduler (08:45 IST) ΓÇª")
        await nse_margin_scheduler.start()

        log.info("[14] Loading exchange holidays into market_hoursΓÇª")
        try:
            holidays_loaded = await load_exchange_holidays_from_db()
            if holidays_loaded:
                log.info("[14] Γ£ô Exchange holidays loaded for market state checks")
            else:
                log.warning("[14] Could not load holidays from database; using empty set")
        except Exception as exc:
            log.warning(f"[14] Holiday loading failed ({exc})")

        log.info("[15] Starting Super Order monitor (Target + SL + Trailing) ΓÇª")
        await start_super_order_monitor()

        log.info("[16] Starting EOD closed-position archiver (16:00 IST) ΓÇª")
        await eod_closed_position_archiver.start()

        log.info("[17] Starting charge calculation scheduler (16:00 IST NSE/BSE, 00:00 IST MCX) ΓÇª")
        await charge_calculation_scheduler.start()

        log.info("[18] Starting market timing controller (auto START/STOP) ΓÇª")
        await market_timing_controller.start()

        log.info("ΓöÇΓöÇΓöÇ Application ready ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ")

    except Exception as exc:
        log.exception(f"Startup failed: {exc}")
        raise

    yield  # ΓöÇΓöÇ Application runs here ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ

    log.info("ΓöÇΓöÇΓöÇ Shutdown ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ")
    ...
    await close_db()
    log.info("ΓöÇΓöÇΓöÇ Shutdown complete ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ")

...
Symbol Mapping:
  BSE SENSEX ΓåÆ NSE NIFTY
  BSE BANKEX ΓåÆ NSE BANKNIFTY

Margin Formula:
  span_margin     = price_scan ├ù quantity          [for cvf == 1.00 (equities)]
...
_BSE_TO_NSE_MAPPING = {
    'SENSEX': 'NIFTY',       # BSE Sensex ΓåÆ NSE Nifty 50
    'BANKEX': 'BANKNIFTY',    # BSE BankEx ΓåÆ NSE BankNifty
}
    ...
    try:
        ...
        for bse_symbol, nse_symbol in _BSE_TO_NSE_MAPPING.items():
            ...
            if nse_span:
                ...
                mapped_count += 1
                log.debug(f"Mapped BSE {bse_symbol} ΓåÉ NSE {nse_symbol} (ELM: {elm_pct}%)")
            else:
...
async def get_margin(
    ...
    # SPAN margin = price_scan ├ù quantity (cvf typically 1.0 for equities)
    span_margin = entry.price_scan * quantity
    
    # ELM margin = ref_price ├ù quantity ├ù elm_pct / 100

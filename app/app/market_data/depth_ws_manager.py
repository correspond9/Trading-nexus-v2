...
=====================================
Full Market Depth WebSocket ΓÇö 20-Level bid/ask for the 3 index underlyings.
Connects to wss://depth-api-feed.dhan.co/twentydepth
Max 50 instruments per connection ΓÇö 3 indexes fits easily in one connection.
...
# Token ΓåÆ pending depth (bid and ask built separately, merged on both received)
...
class _DepthWSManager:
    ...
    async def _run_forever(self) -> None:
        while True:
            ...
            try:
                ...
                log.info("Depth WS: ConnectingΓÇª")
                async with websockets.connect(
                    ...
                    try:
                        ...
                        log.error(f"Depth WS: Server closed before subscribe ΓÇö {exc}")
                        raise
                ...
                log.warning(f"Depth WS: Connection closed ΓÇö {exc}")
            except asyncio.CancelledError:
                return
            except Exception as exc:
                log.error(f"Depth WS: Error ΓÇö {exc}")
            finally:
                self._connected = False
                self._ws = None
            ...
            log.info(f"Depth WS: Reconnecting in {backoff}sΓÇª")
    ...
    async def _handle_binary(self, data: bytes) -> None:
        ...
        Followed by 20 ├ù 16-byte packets: float64 price, uint32 qty, uint32 orders
        """
        if len(data) < 12:
            return

...
Two-layer rate limiting for all DhanHQ REST calls:
  ...
  Layer 1 ΓÇö SlidingWindowRateLimiter
    Per-endpoint sliding-window limiters.  Callers can still use these
    directly, but should prefer DhanHttpClient (Layer 2).

  Layer 2 ΓÇö DhanHttpClient  (universal gateway)
    The single httpx wrapper that EVERY outbound DhanHQ REST call must go
    through.  It:
      ΓÇó Resolves the correct per-endpoint limiter from the URL path
      ΓÇó Awaits it before sending (so no call ever escapes rate control)
      ΓÇó Tracks counters: total calls, per-endpoint calls, throttle delays,
        and HTTP-error counts
      ΓÇó Logs a WARNING when a call had to be delayed (throttle event)
...
Rate limits (DhanHQ v2 DATA APIs):
  /optionchain          ΓåÆ 1 req / 3 sec
  /marketfeed/ltp|ΓÇª     ΓåÆ 1 req / sec
  everything else       ΓåÆ 1 req / sec  (conservative fallback)
"""
import asyncio
import time
import logging
...
# ΓöÇΓöÇ Per-endpoint singleton limiters ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ

# POST /marketfeed/ltp|ohlc|quote  ΓåÆ 1 req/sec
market_quote_limiter = SlidingWindowRateLimiter(
    max_calls=1, window_seconds=1.0, name="market_quote"
)

# POST /optionchain  ΓåÆ conservative: 1 req / 4 sec
...
class _CallStats:
    def __init__(self):
        ...
        self.error_counts:     dict[int, int] = {}   # HTTP status ΓåÆ count
        self.per_endpoint:     dict[str, int] = {}   # path prefix ΓåÆ count
...
class DhanHttpClient:
    ...
    async def _call(
        ...
        from app.credentials.credential_store import get_rest_headers, get_client_id
        limiter = _resolve_limiter(path)
        waited  = await limiter.acquire()
        if waited > 0:
            log.warning(
                f"[DhanClient] Throttled {path!r} ΓÇö waited {waited:.3f}s "
        ...
        if resp.status_code >= 400:
            log.error(
                f"[DhanClient] {method} {path} ΓåÆ HTTP {resp.status_code}: "
                f"{resp.text[:200]}"
            )
    ...
    def get_stats(self) -> dict:
        """Live rate-limit metrics ΓÇö served by GET /admin/rate-limits."""
...
# Singleton ΓÇö import this everywhere instead of creating httpx.AsyncClient directly

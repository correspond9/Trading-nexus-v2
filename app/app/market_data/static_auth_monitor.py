...
class StaticAuthMonitor:
    ...
    On failure: switch auth_mode ΓåÆ auto_totp, log reason, notify admin endpoint
    """
    ...
    async def start(self) -> None:
        ...
        if not is_static_configured():
            log.debug("[StaticAuthMonitor] Static credentials not configured ΓÇö skipping.")
    ...
    async def record_failure(
        ...
        log.warning(
            "[StaticAuthMonitor] Static IP auth failure (%s) ΓÇö count: %d/%d",
            reason_detail,
            self._failure_count,
    ...
    async def _trigger_fallback(self, reason: str) -> None:
        ...
        log.error(
            "[StaticAuthMonitor] Threshold reached. Switching auth_mode ΓåÆ auto_totp. "
            "Reason: %s",
            reason,
        )
    ...
    async def reset_failures(self) -> None:
        ...
        log.info(
            ...
            "Previous count: %d/3 ΓÇö Ready for manual reattempt.",
            prev_count,
        )
    ...
    async def _monitor_loop(self) -> None:
        ...
        while True:
            ...
            if not is_static_configured() or get_active_auth_mode() != "static_ip":
                log.debug("[StaticAuthMonitor] Static auth not active ΓÇö skipping check.")
                continue

            # Log current failure state for observability
            log.debug(
                "[StaticAuthMonitor] Health check ΓÇö failure_count=%d/%d, "
                "monitoring=%s",
                self._failure_count,
                self._failure_threshold,

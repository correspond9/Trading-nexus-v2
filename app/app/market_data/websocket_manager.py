...
Exponential backoff reconnection ΓÇö max 10 attempts then 1-hour cooldown.
"""
import asyncio
import json
import logging
...
# ΓöÇΓöÇ Global connect gate (prevents WS handshake storms) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
# ΓöÇΓöÇ Reconnection parameters ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
_MAX_ATTEMPTS  = 10
_BACKOFF_BASE  = 5      # seconds
_BACKOFF_MAX   = 120
_COOLDOWN_SEC  = 3600   # 1 hour after max attempts exhausted

# ΓöÇΓöÇ Request codes (DhanHQ v2) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
# ΓöÇΓöÇ Feed response codes ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
_FC_TICKER   = 2
_FC_QUOTE    = 4
_FC_FULL     = 8
_FC_PREV_CLOSE = 6
...
class _SingleWSConnection:
    """One WebSocket connection ΓÇö handles one of the 5 slots."""
    ...
    async def _run_forever(self) -> None:
        while True:
            ...
            try:
                ...
                log.info(f"WS-{self.slot}: ConnectingΓÇª")
                async with websockets.connect(
                    ...
                    try:
                        ...
                        log.error(f"WS-{self.slot}: Server closed before subscribe ΓÇö {exc}")
                        raise
                    ...
                    try:
                        ...
                        log.error(f"WS-{self.slot}: Resubscribe failed ΓÇö {exc}")
                        raise
                ...
                log.warning(f"WS-{self.slot}: Connection closed ΓÇö {exc}.")
                ...
                if "HTTP 429" in msg:
                    rate_limited = True
                log.error(f"WS-{self.slot}: Error ΓÇö {exc}")
            finally:
                self._connected = False
                self._ws = None
            ...
            log.info(f"WS-{self.slot}: Reconnecting in {backoff}s (attempt {self._attempts})ΓÇª")
    ...
    async def _resubscribe_all(self) -> None:
        ...
        log.info(
            f"WS-{self.slot}: Subscribing {len(tokens)} tokensΓÇª "
        ...
        for i in range(0, len(tokens), cfg.max_msg_instruments):
            ...
            try:
                ...
                log.warning(
                    ...
                    f"(batch={i//cfg.max_msg_instruments + 1}, size={len(chunk)}) ΓÇö {exc}"
                )
                raise
    ...
    async def _handle_binary(self, data: bytes) -> None:
        ...
        if response_code == _FC_DISCONNECT:
            ...
            log.warning(f"WS-{self.slot}: Server disconnect packet ΓÇö code {disc_code}")
            return
        ...
        try:
            ...
            log.debug(f"WS-{self.slot}: Parse error for token {security_id} ΓÇö {exc}")
...
def _parse_full_packet(data: bytes, security_id: int, code: int) -> dict | None:
    ...
    if code == _FC_FULL and len(data) >= 162:
        # Full packet layout ΓÇö struct '<BHBIfHIfIIIIIIffff100s' (162 bytes)
        # Offsets: B(1)+H(2)+B(1)+I(4)=8 ΓåÆ LTP(f,4), H(2) ΓåÆ LTQ, I(4) ΓåÆ LTT ΓÇª
        ...
        # 5-level market depth: 5 ├ù 20 bytes starting at offset 62
        bid_depth = []
        ask_depth = []
        for i in range(5):
            off = 62 + i * 20
        ...
        # Ticker packet: '<BHBIfI' (16 bytes) ΓÇö LTP at offset 8, LTT at offset 12
        ltp   = struct.unpack_from("<f", data, 8)[0]
        ...
        return {
            ...
            "ltp": round(ltp, 2),
            "ltt": ltt,
            # Other fields absent ΓÇö tick_processor merges with existing row
        }
...
class _WebSocketManager:
    ...
    async def reconnect_all(self) -> None:
        """Token rotation ΓÇö graceful reconnect of all connections."""
        log.info("Reconnecting all WebSocket connections (token rotation)ΓÇª")
        for conn in self._conns:
    ...
    def get_status(self) -> list[dict]:
        """Alias for connection_status ΓÇö used by Admin router."""

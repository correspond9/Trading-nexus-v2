...
GET  /margin/span-data?symbol=NIFTY   (admin/debug ΓÇö returns raw SPAN cache entry)
...
Formula:
    Option buyer  ΓåÆ total = premium           (= ltp ├ù quantity)
    Option seller ΓåÆ total = SPAN + Exposure
    Futures       ΓåÆ total = SPAN + Exposure

Where:
    SPAN margin  = priceScan (from SPAN┬« file) ├ù quantity
    Exposure     = ref_price ├ù quantity ├ù ELM% / 100  (from AEL file)
"""
import logging
...
def _extract_underlying(symbol: str) -> str:
    ...
    E.g.  "NIFTY24FEB25000CE" ΓåÆ "NIFTY"
          "BANKNIFTY"         ΓåÆ "BANKNIFTY"
          "RELIANCE"          ΓåÆ "RELIANCE"
    """
    ...
    for suffix in ("CE", "PE"):
        if sym.endswith(suffix):
            sym = sym[:-2]
            break
    # Strip trailing digit-only expiry/strike (e.g. 24FEB25000 ΓåÆ just letters)
    import re
...
async def calculate_margin_endpoint(body: MarginCalcRequest, request: Request):
    ...
    # ΓöÇΓöÇ Resolve LTP if price not supplied ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    if price == 0.0:
        price = 0.0   # margin will be 0 ΓÇö caller should supply price
    ...
    # ΓöÇΓöÇ NSE SPAN + ELM margin calculation ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
# ΓöÇΓöÇ Admin / Debug endpoints ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
async def get_span_data_endpoint(symbol: Optional[str] = Query(None)):
    ...
    GET /margin/span-data/?symbol=NIFTY   ΓåÆ single symbol
    GET /margin/span-data/                ΓåÆ all symbols (may be large)
    """
    store = get_store()
    if not store.ready:
        return {
...
async def trigger_nse_refresh():
    ...
    """
    log.info("Manual NSE margin refresh triggered via API ΓÇª")

"""
app/routers/watchlist.py  (v2 ΓÇö simplified flat API matching frontend)
GET  /watchlist/{user_id}          ΓåÆ [{symbol, token, exchange, ...}]
POST /watchlist/add                ΓåÆ {user_id, token, symbol, exchange}
POST /watchlist/remove             ΓåÆ {user_id, token}
"""
import logging
import uuid
...
async def get_watchlist(user_id: str, request: Request):
    ...
    if not wl:
        # No watchlist yet ΓÇö return empty list
        return {"data": []}

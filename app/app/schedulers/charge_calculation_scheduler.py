...
class ChargeCalculationScheduler:
    ...
    async def start(self):
        ...
        logger.info("≡ƒöä Charge calculation scheduler started")
    ...
    async def stop(self):
        ...
        if self._task:
            ...
            try:
                ...
                pass
        logger.info("ΓÅ╣∩╕Å Charge calculation scheduler stopped")
    ...
    async def _check_and_run(self):
        ...
        if (current_time >= NSE_BSE_CHARGE_TIME and 
            ...
            logger.info("ΓÅ░ Running NSE/BSE charge calculation (4:00 PM IST)")
        ...
        if (current_time >= MCX_CHARGE_TIME and 
            ...
            logger.info("ΓÅ░ Running MCX charge calculation (12:00 AM IST)")
    ...
    async def run_once(self, exchanges: Optional[List[str]] = None) -> dict:
        ...
        logger.info(f"≡ƒº« Starting charge calculation for exchanges: {exchanges or 'ALL'}")
        ...
        try:
            ...
            if total == 0:
                logger.info("Γ£à No positions to process")
                return {'processed': 0, 'errors': 0, 'skipped': 0}
            
            logger.info(f"≡ƒôè Found {total} positions to process")
            ...
            logger.info(f"Γ£à Charge calculation complete: {processed} processed, {errors} errors, {duration:.2f}s")
    ...
    async def _calculate_and_update_charges(self, position: dict, pool):
        ...
        try:
            ...
            logger.debug(f"Γ£ô Calculated charges for position {position['position_id']}: Γé╣{charges['total_charges']:.2f}")

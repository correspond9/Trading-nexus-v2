import paramiko
from io import StringIO

KEY = """-----BEGIN OPENSSH PRIVATE KEY-----

-----END OPENSSH PRIVATE KEY-----"""

ssh = paramiko.SSHClient()
ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
key = paramiko.Ed25519Key.from_private_key(StringIO(KEY))
ssh.connect("72.62.228.112", username="root", pkey=key, timeout=20)

stdin, stdout, stderr = ssh.exec_command("docker ps --format \"{{.Names}}\" | grep '^db-' | head -1")
container = stdout.read().decode().strip()

cmd = f"""docker exec {container} psql -U postgres -d trading_terminal -c "\\d paper_positions" """
stdin, stdout, stderr = ssh.exec_command(cmd)
print(stdout.read().decode())

ssh.close()

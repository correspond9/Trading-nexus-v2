"""
QA Test Script - Phase 2: User Role Testing
============================================
...
# Test credentials
USERS = {
    "SUPER_ADMIN": {"user_id": "9999999999", "password": "admin123"},
    "ADMIN": {"user_id": "8888888888", "password": "admin123"},
    "USER": {"user_id": "7777777777", "password": "user123"},
    "SUPER_USER": {"user_id": "6666666666", "password": "super123"},
}
...
ENDPOINTS_TO_TEST = [
    ...
    # Watchlist (should be accessible to all)
...
async def login(client: httpx.AsyncClient, user_id: str, password: str) -> Dict[str, Any]:
    ...
    if response.status_code == 200:
        data = response.json()
        return {"success": True, "data": data, "cookies": response.cookies}
    else:
...
async def test_role(role: str, credentials: Dict[str, str]):
    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        ...
        login_result = await login(client, credentials["user_id"], credentials["password"])
        ...
        print(f"   User: {login_result['data'].get('user_id')}")
        print(f"   Role: {login_result['data'].get('role')}")
        print(f"   Name: {login_result['data'].get('name', 'N/A')}")
        
        # Update client cookies
        client.cookies.update(login_result["cookies"])
        ...
        for endpoint_config in ENDPOINTS_TO_TEST:
            ...
            print(f"   {status_icon} [{result['status_code']}] {method:4} {endpoint_url:50} - {feature}")
...
| Role | User ID | Login Status | Actual Role |
|------|---------|--------------|-------------|
"""
    ...
    for result in results:
        ...
        report += f"| {role} | {user_id} | {login_status} | {actual_role} |\n"
    ...
    for result in results:
        ...
        report += f"- **User ID:** {user_data.get('user_id')}\n"
        report += f"- **Name:** {user_data.get('name', 'N/A')}\n"
        report += f"- **Role:** {user_data.get('role')}\n"
        report += f"- **Balance:** ₹{user_data.get('balance', 0):.2f}\n\n"
